...
subtitle: "For roundtable discussion with Somerset"
...
-   Somerset has selected amongst the fewest parameters of all schemes, with only 30 of the 92 used. The lowest count is Torbay at 17 whilst the highest is PAH and Hinchingbrooke, which have both selected 84.
-   It has has one of the highest levels of ambition, with an average 21% reduction selected in its inputs. The most ambitious scheme on average is Hillingdon (34% reduction) and the least is PAH (6% reduction).
-   It is one of the most confident schemes in its inputs, with an average confidence interval of 6 percentage points around the central estimate. By contrast, the most certain scheme, PAH, has an average of 2 percentage points and the least, James Paget, has an average of 22 percentage points.
-   Perhaps unsurprisingly given its relative ambition, Somerset has amongst the largest proportion of its parameters which meet or exceed the ambition of the corresponding National Expert Elicitation value: 30.0% of its inputs do so, compared to 35.3% for Torbay and 2.9% for Frimley.
-   There are 46 parameters which Somerset did not use where at least half of the other 17 schemes did provide an input. These include *Falls Related Admissions* which was selected by all of the 16 other schemes.
-   It has 2 parameters which are significantly more ambitious than its average input of a 21% reduction: *Virtual Wards Admission Avoidance (Acute Respiratory Infection)* (85% reduction) and *Enhanced Recovery (Breast)* (65% reduction)

# Overview

## Purpose of this report

The purpose of this report is to compare individual scheme's inputs to the NHP Demand and Capacity Model. We consider 4 areas by which to compare the inputs: parameter coverage, ambition, certainty, and credibility given the National Elicitation Exercise (NEE):

<table>
<thead>
<tr class="header">
<th><p>Area of analysis</p></th>
<th><p>Scheme-level</p></th>
</tr>
</thead>
<tbody>
<tr class="odd">
<td><p><strong>Parameter coverage</strong></p></td>
<td><p>What proportion of parameters has each scheme selected?</p></td>
</tr>
<tr class="even">
<td><p><strong>Parameter ambition</strong></p></td>
<td><p>What is the average ambition of each schemes selected parameters?</p></td>
</tr>
<tr class="odd">
<td><p><strong>Parameter certainty</strong></p></td>
<td><p>What is the is the average certainty of each schemes selected parameters?</p></td>
</tr>
<tr class="even">
<td><p><strong>Parameter credibility</strong></p>
<p><strong>(NEE comparison)</strong></p></td>
<td><p>How do each schemes parameter selections compare to the the NEE mid-point?</p></td>
</tr>
</tbody>
</table>
...
```{r scheme}
selected_scheme_code <- "RH5"
selected_scheme_name <- "Somerset"

```

Below we present scheme-level analyses. `r selected_scheme_name` is highlighted in blue.
...
![](scheme_output_reports/images/somerset_8_1.png)

### 8.2 Top parameters for admission avoidance

![](scheme_output_reports/images/somerset_8_2.png)

### 8.3 Top parameters for reduced bed days due to avoidance

![](scheme_output_reports/images/somerset_8_3.png)

### 8.4 Top parameters for reduced bed days due to efficiencies

![](scheme_output_reports/images/somerset_8_4.png)

### 9. Compound annual growth rates from BAU shifts

| Measure                                                 | `r selected_scheme_name` |
|-----------------------------------------------------|-------------------|
| Assumed BAU admissions avoided as CAGR                  | -0.31%                   |
| Assumed BAU resultant beddays avoided as CAGR           | -0.71%                   |
| Assumed BAU productivity as CAGR for overall LOS impact | -0.4%                    |
...
mitigator_coverage_by_scheme |> 
  highlight_schemes(selected_scheme_code) |> 
  ggplot() +
...
The following table indicate unselected parameters for `r selected_scheme_name`.
...
missing_mitigators_by_scheme |> 
  filter(scheme_code == selected_scheme_code) |>
...
The following table indicates the parameters set by `r selected_scheme_name` that were significantly more ambitious than its other inputs.
