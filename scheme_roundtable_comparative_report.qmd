...
subtitle: "For roundtable discussion with Airedale"
...
```{r }
markdown::mark("scheme_text_markdown/airedale.md", output = FALSE, template = FALSE)
```
...
<tr class="even">
<td><p><strong>Parameter value</strong></p></td>
<td><p>What is the average value of each schemes selected parameters?</p></td>
</tr>
<tr class="odd">
...
```{r scheme}
selected_scheme_code <- "RCF"
selected_scheme_name <- "Airedale"
...
![](scheme_output_reports/images/airedale_8_1.png)

### 8.2 Top parameters for admission avoidance

![](scheme_output_reports/images/airedale_8_2.png)

### 8.3 Top parameters for reduced bed days due to avoidance

![](scheme_output_reports/images/airedale_8_3.png)

### 8.4 Top parameters for reduced bed days due to efficiencies

![](scheme_output_reports/images/airedale_8_4.png)

### 9. Compound annual growth rates from BAU shifts

| Measure                                                 | `r selected_scheme_name` |
|----------------------------------------------------|--------------------|
| Assumed BAU admissions avoided as CAGR                  |                   |
| Assumed BAU resultant beddays avoided as CAGR           |                    |
| Assumed BAU productivity as CAGR for overall LOS impact |                     |
...
Below, for each scheme, we can see the average value over its selected parameters in ascending order. Note that the lower the number, the greater extent of mitigation the given scheme has assumed in its
selections given that the lower values corresponds to less activity.
...
The following table indicates the parameters set by `r selected_scheme_name` that assumed significantly more mitigation than its average.
