...
# subtitle: "For roundtable discussion with Frimley"
subtitle: "For roundtable discussion with Leighton"
...
The chart shows, for each group, the proportion of scheme inputs which fall into each of these thresholds. We can see that across all parameter selections 96 (10.5%) of scheme inputs were high ambition, 439 (48.1%) were medium ambition, and 378 (41.4%) were low ambition.
...
The chart below indicates that, across all groups, 268 (29.4%) of scheme inputs were believed by schemes to be highly certain, 320 (35.0%) were believed to be medium certainty, and 325 (35.6%) were believed to be low certainty.
...
It indicates that, across all parameter selections, 152 (16.6%) were as or more ambitious than the NEE mid-point, and 667 (73.1%) were less ambitious, with the remainder (94,10.3%) not being covered in the NEE exercise.
...
Below we present scheme-level analyses. The schemes in this roundtable are highlighted as follows:
...
![](scheme_output_reports/images/leighton_8_1.png)
...
![](scheme_output_reports/images/leighton_8_2.png)
...
![](scheme_output_reports/images/leighton_8_3.png)
...
![](scheme_output_reports/images/leighton_8_4.png)
...
| Measure                                                 | Leighton |
|---------------------------------------------------------|----------|
| Assumed BAU admissions avoided as CAGR                  | -0.2%    |
| Assumed BAU resultant beddays avoided as CAGR           | -0.25%   |
| Assumed BAU productivity as CAGR for overall LOS impact | -0.3%    |
...
<!-- The following table indicate unselected parameters for Frimley. -->

The following table indicate unselected parameters for Leighton.

<!-- The following table indicate unselected parameters for Hinchingbrooke, King's Lynn, West Suffolk, and James Paget. -->

<!-- The following table indicate unselected parameters for Hillingdon. -->

We have filtered this to where at least half of the total `r n_schemes` have selected the parameter.

```{r unset_parameters}
missing_mitigators_by_scheme |> 
  # Frimley
  # filter(scheme_code == "RDU") |>
  
  # Leighton
  filter(scheme_code == "RBT") |>
  
  # West Suffolk, James Paget et al
...
<!-- The following table indicates the parameters set by Frimley that were significantly more ambitious than its other inputs. -->

The following table indicates the parameters set by Leighton that were significantly more ambitious than its other inputs.

<!-- The following table indicates the parameters set by the four schemes that were significantly more ambitious than their other inputs. -->

<!-- The following table indicates the parameters set by Hillingdon that were significantly more ambitious than their other inputs. -->
...
-   Across mitigatable activity groups, coverage of parameters ranges from 51.8% (*Emergency department and acute medicine activity*) to 73.5% (*Planned medical activity (adult)*).

<!-- -   Frimley has selected amongst the fewest parameters of all schemes at 34. This contrasts to PAH and Hinchingbrooke which have both selected 84. -->

-   Leighton has selected amongst the fewest parameters of all schemes at 29. This contrasts to PAH and Hinchingbrooke which have both selected 84.

<!-- -   Hinchingbrooke has selected the joint most parameters (84) of all schemes. West Suffolk has selected the least of the four schemes (58) but is still in the middle position of all schemes (9th out of 17). -->

<!-- -   Hillingdon has selected the 7th fewest parameters of all schemes, having selected 35 of the 92 parameters. This compares to 84 that both Hinchingbrooke and PAH have selected -->

### Parameter ambition

-   *Planned surgical activity (adult)* contains the largest proportion of high ambition assumptions (52, 20.6%), whilst *Hospital activity amenable to psychiatric liaison and community psychiatry* contains the largest proportion of low ambition assumptions (18, 47.4%).

<!-- -   Frimley has the fourth lowest ambition of all schemes, with an average input of 0.9 -->

-   Leighton's average ambition amongst its parameters is around the middle of all schemes, at 0.82.

<!-- -   We can see that the four schemes' ambition ranges between 0.91 (West Suffolk) and 0.76 (James Paget). these all fall in the range of average ambitions shown by all schemes (0.96 to 0.66). -->

<!-- -   Hillingdon has the most ambitious average parameter input at 0.66. The next highest is Watford at 0.75. -->

### Parameter certainty

-   The grouping with the largest proportion of high certainty assumptions is *Planned medical activity (adult)* (17, 34.0%) whilst the one with largest proportion of low certainty assumptions is *Emergency department and acute medicine activity* (78, 44.3%)

<!-- -   Frimley has one of the highest levels of certainty in its inputs, with an average confidence interval of 0.06. -->

-   Leighton has the 6th largest level of uncertainty in its inputs, with an average confidence interval of 0.15

<!-- -   James Paget has shown the largest average uncertainty range in its selections (0.22) of all schemes, with the remaining three schemes being around the middle of all schemes and closely clustered to one another (0.11 to 0.14) -->

<!-- -   Hillingon has the 7th highest certainty in its inputs, with an average range of 0.11 -->

### NEE credibility

-   A large majority (73.1%) of all assumptions across all schemes are less ambitious than the NEE mid-point, with only 16.6% being as or more ambitious.

-   For each individual scheme that has model runs, a minority of its selections are as or more ambitious

<!-- -   Frimley has the lowest proportion of its parameters meeting or exceeding NEE ambition of all schemes at only 2.9%. -->

-   Leighton is around the middle of all schemes in the proportion of its inputs that meet or exceed the ambition of the NEE (17.2%)

<!-- -   Of the four schemes, James Paget has the largest proportion inputs that are as or more ambitious (28.8%), whilst West Suffolk has the smallest (3.4%) -->
...
<!-- -   Frimley has 41 unselected parameters which were used by at least half of all schemes, with, for example, *Outpatient Followup Appointment Reduction (Children, Surgical)* having been used by 14 of the 17 schemes. -->

-   It has two particularly ambitious parameters, both of which come from the *Planned medical activity (adult)* grouping.

-   Leighton has 42 unselected parameters which were used by at least half of all schemes, with some of these having been used by nearly all the schemes, e.g. *Outpatient Convert to Tele-Attendance (Adult, Non-Surgical)* was used by 15 schemes and *Emergency Admission of Older People* by 14.

<!-- -   It has two particularly ambitious parameters, both of which come from the *Planned surgical activity (adult) grouping* grouping. -->
...
<!-- -   Hillingdon has 38 unselected parameters which were used by at least half of all schemes, with *Frail Elderly Admissions (Intermediate Frailty Risk)* having been selected by all 16 of the other schemes -->
